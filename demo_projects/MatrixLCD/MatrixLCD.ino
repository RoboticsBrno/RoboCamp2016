...
void loadBufferLong(int ascii){
    if (ascii >= 0x20 && ascii <=0x7f){
        ...
        for (byte x=0; x<count;x++){
          int v = analogRead(0);
          scrollDelay = map(v, 0, 1023, 5, 1023);
...
void printBufferLong(){
  for (int a=0;a<7;a++){                    // Loop 7 times for a 5x7 font
    ...
    setRow(0,a,flip(y));                       // Send row to relevent MAX7219 chip
    x = bufferLong [a*2];                   // Get low buffer entry
    y = (x>>24);                            // Mask off second character
    setRow(1,a,flip(y));                       // Send row to relevent MAX7219 chip
    y = (x>>16);                            // Mask off third character
    setRow(2,a,flip(y));                       // Send row to relevent MAX7219 chip
    y = (x>>8);                             // Mask off forth character
    setRow(3,a,flip(y));                       // Send row to relevent MAX7219 chip
  }
}
